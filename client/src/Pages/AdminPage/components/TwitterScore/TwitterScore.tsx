...
const TwitterScore: FC<TwitterScoreProps> = ({ value, className = '' }) => {
  ...
  useEffect(() => {
    ...
    const calculateGradientColors = () => {
      const red = Math.min(255, Math.max(0, 170 - value * STEP));
      const green = Math.min(170, Math.max(0, value * STEP));
      const redStop = Math.min(255, Math.max(0, 170 - value * STEP - 20));
      const greenStop = Math.min(170, Math.max(0, value * STEP + 20));

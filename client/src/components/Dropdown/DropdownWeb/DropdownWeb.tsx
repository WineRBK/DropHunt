...
const DropdownWeb: FC<DropdownWebProps> = ({ handleWeb }) => {
  ...
  const [content, setContent] = useState<WebData[] | null>(null);
  const [searchText, setSearchText] = useState<string>('');
  const [filteredList, setFilteredList] = useState<WebData[] | null>(null);
  ...
  return (
    <div className={s.dropdown__wrapper} ref={dropdownRef}>
      ...
      <div className={cn(s.dropdown__content, { [s.activeContent]: active === true })}>
        ...
        <ul>
          {filteredList &&
            filteredList.map((item: WebData) => (
              <li
                key={item.id}
                className={cn({ [s.activeItem]: webList.includes(item.name) })}
                tabIndex={0}
                role="button"
                onClick={() => changeWebList(item.name)}>
                {item.name}
              </li>
            ))}
        </ul>
